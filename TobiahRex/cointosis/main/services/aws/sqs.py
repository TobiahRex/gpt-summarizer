...
class SQSService(object):
    ...
    def extract_s3_data_from_msg(self, msg_body):
        """
        Given a SQSService message body from a S3 stream return the associated bucket & key of the created/updated item
        :param msg_body: SQSService message body from a S3 stream
        :return: bucket (str) and key (str)
        """
        try:
            json_msg = json.loads(msg_body)
            record = json_msg.get('Records', {})
            path = record.get('s3', {}).get('object', {}).get('key')
            bucket = record.get('s3', {}).get('bucket', {}).get('name')
            return bucket, path
        except Exception as err:
            print(
                'Error Extracting S3 Data From Message. Error: {}'.format(err))
    ...
    def list_queue_tags(self, queue_url):
        return self.sqs_client.list_queue_tags(QueueUrl=queue_url)

    def get_queue_attributes(self, queue_name, attributes=None):
        params = {
            'QueueUrl': self.__get_queue_url(queue_name)
        }
        if attributes is not None:
            params['AttributeNames'] = attributes
        return self.sqs_client.get_queue_attributes(**params).get('Attributes')
